-- ============================================
-- MOOMINGLE COMPLETE DATABASE MIGRATION V2
-- ============================================
-- This migration sets up the entire database schema from scratch
-- Run this in your Supabase SQL Editor
-- FIXED: Properly handles existing listings table structure
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'ℹ️  status column already exists in listings table';
  END IF;
END $$;
...
CREATE POLICY "Sellers can insert muzzle prints for their listings"
  ...
  WITH CHECK (
    EXISTS (
      ...
      AND (listings.seller_id = auth.uid() OR listings.seller_id IS NULL)
    )
  );
...
-- Seller performance view (only for listings with seller_id)
...
BEGIN
  ...
  RAISE NOTICE '   • listings (added seller_id and status columns)';
  ...
  RAISE NOTICE '';
  RAISE NOTICE '⚠️  NOTE: Existing listings have seller_id = NULL';
  RAISE NOTICE '   You can link them to users by running:';
  RAISE NOTICE '   UPDATE listings SET seller_id = (SELECT id FROM profiles WHERE full_name = listings.seller_name LIMIT 1) WHERE seller_name IS NOT NULL;';
END $$;
